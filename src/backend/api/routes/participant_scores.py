...
router = APIRouter(
    prefix="/events/{event_id}/participants/{participant_id}/scores",
    tags=["scores"]
)
...
def post_score(event_id: int, participant_id: int, score: ParticipantScoreSchema, db=Depends(get_db)):
