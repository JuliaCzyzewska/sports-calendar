...
from src.backend.services.shared_queries import (
    fetch_participants_by_event,
    fetch_results_by_event,
)
...
def row_to_event_response(
    row, participants: list[ParticipantResponse], results: list[EventResultResponse]
) -> EventResponse:
...
def get_all_events(db) -> list[EventResponse]:
    ...
    return [
        row_to_event_response(
            row,
            participants_by_event.get(row["event_id"], []),
            results_by_event.get(row["event_id"], []),
        )
        for row in rows
    ]
...
def get_one_event(event_id: int, db) -> EventResponse:
    cur = db.cursor()
    row = cur.execute(EVENTS_QUERY + " WHERE e.id = ?", [event_id]).fetchone()
    if not row:
...
def post_event(event: EventCreate, db) -> EventResponse:
    ...
    competition = cur.execute(
        "SELECT slug FROM competitions WHERE slug = ?", [event.competition_slug]
    ).fetchone()
    if not competition:
    ...
    if event.venue_id:
        venue = cur.execute(
            "SELECT id FROM venues WHERE id = ?", [event.venue_id]
        ).fetchone()
        if not venue:
