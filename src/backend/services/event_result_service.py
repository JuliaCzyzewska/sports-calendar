...
def validate_event_can_add_results(event_id: int, db):
    ...
    if event["status"] == "cancelled":
        raise HTTPException(
            status_code=409, detail="Cannot add results to a cancelled event"
        )
...
def post_result(event_id: int, result: EventResultCreate, db) -> EventResultResponse:
    ...
    if existing:
        detail = (
            f"Result for category '{result.category}' already exists for this event"
            if result.category
            else "Result already exists for this event"
        )
