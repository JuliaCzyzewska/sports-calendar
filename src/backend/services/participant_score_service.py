...
def post_score(event_id: int, participant_id: int, score: ParticipantScoreSchema, db) -> ParticipantScoreSchema:
    # validate FKs
    participant = db.execute(
        "SELECT id FROM participants WHERE id = ? AND _event_id = ?", [participant_id, event_id]
    ).fetchone()
    if not participant:
    ...
    if existing:
        raise HTTPException(status_code=409, detail=f"Score '{score.score_label}' already exists for this participant")

    try:
        db.execute(POST_SCORE_QUERY, [participant_id, score.score_value, score.score_label])
        db.commit()
