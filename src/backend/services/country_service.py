...
def get_all_countries(db) -> list[CountryResponse]:
    ...
    return [
        CountryResponse(
            id=row["id"], abbreviation=row["abbreviation"], name=row["name"]
        )
        for row in rows
    ]
...
def get_country(country_id: int, db) -> CountryResponse:
    row = db.execute(COUNTRIES_QUERY + " WHERE id = ?", [country_id]).fetchone()
    if not row:
...
def post_country(country: CountryCreate, db) -> CountryResponse:
    ...
    if existing:
        raise HTTPException(
            status_code=409, detail="Country with this abbreviation already exists"
        )
    ...
    return CountryResponse(
        id=new_id, abbreviation=country.abbreviation, name=country.name
    )
